...
처음 실행하거나 화면을 실제로 활용하려면 [사용자 매뉴얼](docs/USER_MANUAL.md)부터 확인하세요.
...
- `/`: 데이터 대시보드
- `/draws`: 최신순 회차 목록, 페이지당 20건
- `/draws/1232`: 단일 회차 상세
- `/analysis`: 최근·회차·날짜 범위 기본 통계
- `/analysis/compare`: 최근 기간과 직전 동일 기간 또는 전체 회차의 출현률 비교
- `/analysis/gaps`: 선택 범위의 번호별 출현 간격 요약
- `/analysis/patterns`: AC·인접 간격·수 구성·합계 구간 분포
- `/analysis/relationships`: 번호쌍·3개 조합·기준 번호 동반 출현 관계
- `/analysis/matrix`: 1~45 번호의 7×7 출현 행렬
- `/analysis/matrix/compare`: 직전 동일 기간 대비 출현률 차이 행렬
- `/analysis/similarity`: 과거 당첨 조합쌍의 공통 번호와 회차별 최대 유사 기록
- `/generate`: 조건 기반 무작위 후보 조합 생성
- `/backtests`: 미래 데이터 차단 단일 전략 백테스트
- `/backtests/experiment`: 동일 조건의 반복 전략 비교

회차 목록은 URL의 `page` 쿼리로 이동하며, 상세 화면에서는 저장된 범위 안에서 이전·다음 회차로 이동할 수 있습니다.

기본 통계 화면은 최근 50·100·200·500회와 전체 회차 프리셋을 제공합니다. 시작·끝 회차 또는 시작·끝 날짜를 직접 입력할 수도 있습니다. 결과에는 선택 범위 요약, 번호 합계, 연속번호 포함률, 출현 횟수 상위와 1~45 전체 번호 상세가 표시됩니다. 모든 결과는 과거 기술 통계이며 미래 번호 예측으로 해석하지 않습니다.

기간 비교 화면은 최근 20·30·50·100·200회를 직전 동일 길이 또는 전체 회차와 비교하고 번호별 회차당 출현률과 순위 차이를 표시합니다. 출현 간격 화면은 최근 50·100·200·500회 또는 전체 회차에서 평균·최근 간격과 현재 미출현 회차를 표시합니다. 두 화면 모두 과거 기록의 기술적 비교이며 추세나 다음 출현 시점을 예측하지 않습니다.

조합 패턴 화면은 AC 값, 인접 간격, 소수·합성수·제곱수 개수와 번호 합계 구간을 표시합니다. 번호 관계 화면은 기준 번호의 동반 출현, 상위 번호쌍·3개 조합, 번호 거리와 이전 회차 중복을 표시합니다. 관계 빈도는 추천 점수나 조건부 당첨 확률로 해석하지 않습니다.

번호 행렬 화면은 1~45를 고정된 7×7 위치에 배치해 출현 횟수, 행·열 합계와 대각선 통계를 표시합니다. 기간 차이 행렬은 최근 기간과 직전 동일 길이 기간의 회차당 출현률 차이를 표시합니다. 색상 강도는 선택한 과거 범위 안의 상대적인 크기이며 미래 추세를 뜻하지 않습니다.

조합 유사도 화면은 최근 50·100·200·500회 안의 서로 다른 회차쌍을 비교해 공통 번호 개수 분포를 표시합니다. 최근 20개 회차는 최대 공통 번호 수와 유사했던 이전 회차 링크를 함께 제공합니다. 과거 조합과의 유사성은 미래 당첨 가능성이나 추천 점수가 아닙니다.

번호 생성 화면은 균등 무작위와 과거 빈도 가중 전략, 포함·제외 번호, 홀짝·저고·합계·소수·AC·연속번호와 중복 제한을 지원합니다. 같은 데이터·조건·seed를 사용하면 같은 결과를 재현할 수 있습니다. 생성 결과는 조건을 만족하는 무작위 후보이며 추천 번호나 당첨 예측이 아닙니다.

백테스트 화면은 균등 또는 과거 빈도 가중 전략을 최근 목표 회차들에 순서대로 적용합니다. 각 목표는 해당 회차보다 이전 데이터만 학습하며, 전체 생성 조합의 일치 분포와 목표별 최고 일치 분포를 별도로 표시합니다. 과거 일치 결과는 미래 성과나 실제 당첨 확률을 보장하지 않습니다.

반복 전략 비교 화면은 균등 무작위, 목표 이전 전체 빈도 가중과 최근 N회 빈도 가중 전략을 같은 목표·조합 수·seed로 반복합니다. 조합 수별로 세 전략을 묶어 평균 일치와 목표별 평균 최고 일치를 표시합니다. 조합 수가 다른 행의 최고값은 직접 비교하지 않습니다.

핵심 사용자 경로 E2E 테스트는 설치된 Google Chrome을 사용하며, 테스트가 자체 mock API와 Next.js 개발 서버를 실행하므로 FastAPI와 PostgreSQL은 필요하지 않습니다.
...
데스크톱과 모바일 화면에서 대시보드, 회차 상세·404, 기본 통계와 번호 생성 입력 보존을 확인합니다.
