...
- PostgreSQL 기반 Streamlit 데이터 대시보드와 기본 빈도 화면

SQLite는 제외했으며 PostgreSQL 17을 Docker Compose로 실행합니다. 명세의 고급 분석, 균등·빈도 가중 번호 생성, 반복 백테스트와 Streamlit 초기 읽기 전용 화면까지 구현했습니다. 추가 생성 전략과 Streamlit 데이터 관리·생성·백테스트 화면은 아직 구현하지 않았습니다.
...
브라우저에서 `http://localhost:8501`을 열면 데이터 대시보드와 기본 통계 화면을 확인할 수 있습니다. 현재 화면은 읽기 전용이며 수집·DB 변경·번호 생성·백테스트 실행 기능은 포함하지 않습니다. 기본 통계의 출현 횟수와 미출현 기간은 과거 데이터 설명이며 미래 당첨 가능성을 의미하지 않습니다.
