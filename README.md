...
- PostgreSQL 기반 Streamlit 대시보드·기본 통계·패턴 분석 화면
...
브라우저에서 `http://localhost:8501`을 열면 데이터 대시보드, 기본 통계와 패턴 분석 화면을 확인할 수 있습니다. 패턴 분석은 전체 또는 최근 20·30·50·100회의 7×7 번호 행렬, AC 값, 합계 구간, 대각선과 연속번호 요약을 제공합니다. 현재 화면은 읽기 전용이며 수집·DB 변경·번호 생성·백테스트 실행 기능은 포함하지 않습니다. 모든 통계는 과거 데이터 설명이며 미래 당첨 가능성을 의미하지 않습니다.
