...
브라우저에서 `http://localhost:8501`을 열면 데이터 대시보드와 기본·패턴·관계·기간·간격 분석, 번호 생성 화면을 확인할 수 있습니다. 번호 생성 화면은 균등·전체 빈도·최근 빈도 전략과 포함·제외 번호, 홀짝·고저·합계·소수·AC·중복 조건 및 seed를 지원합니다. 생성 결과는 조건을 통과한 후보와 조합 특성을 표시하며 추천 순위나 당첨 확률이 아닙니다. 수집·DB 변경·백테스트 실행 화면은 아직 포함하지 않습니다.

분석 엔진의 과거 조합 유사도와 행렬 기간 차이는 CLI에서 사용할 수 있지만 Streamlit 화면 연결은 후속 작업입니다.
